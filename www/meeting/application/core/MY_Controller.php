...
abstract class MY_Controller extends CI_Controller {
	...
	protected function verifyUser($event_id, $user, $password) {
		...
		$this->session->set_userdata('officer_event_id', $event_data->id);
		$this->session->set_userdata('officer_id', $booking->id);
		$this->session->set_userdata('officer_login_id', $booking->officer_email);
		$this->session->set_userdata('officer_login_password', $booking->login_password);

		return true;
	}

	protected function check_login() {
		$client_event_id = $this->session->userdata('officer_event_id');
		$client_booking_id = $this->session->userdata('officer_id');
		$client_login_id = $this->session->userdata('officer_login_id');
		$client_login_password = $this->session->userdata('officer_login_password');
	...
	public function logout() {
		$event_id = $this->session->userdata('officer_event_id');
		$this->session->sess_destroy();

		$this->session->set_userdata('officer_event_id', $event_id);
		$event = $this->db
			->where('id', $event_id)
			->get('es_exhibitions')
			->row();
