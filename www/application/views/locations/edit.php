        ...
        <div class="row">
            <div class="col-xs-12">
                <div class="box">
                    ...
                    <div class="box-body">
                        <form action="<?= base_url('locations-edit-submit.html') ?>?id=<?= $this->input->get('id') ?>"
                            ...
                            <div class="row">
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" name="location_title"
                                               value="<?= html_escape($this->formdata->location_title) ?>"
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" name="location_country"
                                               value="<?= html_escape($this->formdata->location_country) ?>"
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" name="location_city"
                                               value="<?= html_escape($this->formdata->location_city) ?>"
                                ...
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        ...
                                        <input type="hidden" id="location_lat" name="location_lat" value="<?= html_escape($this->formdata->location_lat) ?>">
                                        <input type="hidden" id="location_lng" name="location_lng" value="<?= html_escape($this->formdata->location_lng) ?>">
                                        <input type="text" id="location_address" class="form-control" name="location_address" placeholder="Location Address" value="<?= html_escape($this->formdata->location_address) ?>">
...
<script>
	$(function () {
		...
		doFormValidation({
			...
			'urlValidator': "<?php echo base_url("locations-edit-validate.html"); ?>?id=<?= $this->input->get('id') ?>",
	...
	function initlocationsMap() {
		...
		<?php if ($this->formdata->location_lat && $this->formdata->location_lng) {
		echo 'area_map_center = new google.maps.LatLng('.$this->formdata->location_lat.', '.$this->formdata->location_lng.');';
        } ?>
		...
		marker = new google.maps.Marker({
			map:      area_map,
			title:    '<?= $this->formdata->location_address ?>',
			position: {lat: <?= $this->formdata->location_lat ?>, lng: <?= $this->formdata->location_lng ?>},
			draggable: true
		});
		marker.addListener('dragend', function(event) {
			console.log('final position is '+event.latLng.lat()+' / '+event.latLng.lng());
	...
	function add_hall(data) {
		...
		var hall_id = (data && data.hasOwnProperty('id')) ? data.id : '';

		html = '<div class="form-group row" >\
                <div class="col-sm-3">\
                    <input type="hidden" class="form-control location_hall_id" name="hall['+id+'][hall_id]"\
			                value="'+hall_id+'">\
	...
	$(document).on('click', '.remove_hall_btn', function (e) {
		...
		var hall_id = $(this).parents('.form-group').find('.location_hall_id').val();
		...
		$.ajax({
			type:    'post',
			url:     '<?= base_url("locations/check_hall_usage"); ?>',
	...
	foreach ($halls as $hall) {
		echo 'add_hall('.json_encode($hall).');';
	}
	?>
</script>
</body>
