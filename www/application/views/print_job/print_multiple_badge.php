<page>
    ...
    <table style="width: 100%; padding: 5px;margin-top: 15.1mm;">
        <tr>
            <td style="width: 100%;">
                <br>
                <div style="margin-bottom: 0; font-size: 13px; font-family: Helvetica, Arial, sans-serif;"><?= html_escape($badge->full_name) ?></div>
                <?php if (!empty($badge->designation)): ?>
                    <?= html_escape($badge->designation) ?><br>
                <?php endif; ?>
                <?php
                $badge_org = !empty($badge->company) ? trim($badge->company) : '';
                ...
                <br>
                <?php
                if ($badge->nationality == 'Pakistani' || strtoupper($badge->nationality) == 'PAKISTAN') {
                    echo $badge->cnic . ' / ' . $badge->nationality . ' / ' . $badge->barcode_data;
                } else {
                    echo $badge->passport . ' / ' . $badge->nationality . ' / ' . $badge->barcode_data;
                }
                ?>
            </td>
