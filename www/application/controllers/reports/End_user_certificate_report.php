...
class End_user_certificate_report extends MY_Controller
{
	...
	function crd_list()
	{
		...
		$this->load->view('end_user_certificate_report/list');
	}
	...
	function crd_list_datatable()
	{
		...
		$this->datatables
			...
			->add_column('col_action', function ($row) {
				$id = $row['id'];
				$html = '<a href="' . base_url() . 'end_user_certificate_report-list.html?id=' . urlencode(myid($id)) . '">Report</a>';
	...
	{
		$this->load->view('includes/after_login/head');
		$this->load->view('end_user_certificate_report/exhibition_list');
	}
	...
	function crd_exhibition_list_datatable()
	{
		...
		$this->datatables
            ...
            ->add_column('freight_forwader_name', function ($row) {
                ...
                if (isset($data)) {
                    ...
                    if(isset($data->products) && count($data->products) > 0){
                        $html .= rtrim(str_replace('_', ' ', $data->products[0]->official_freight_forwarder ). ',',',');
                    }
                }
            ...
            ->add_column('col_action', function ($row) {
                ...
                $html = 'Certificate not uploaded!';


                if (isset($data)) {
					$data = json_decode($data->form_data);
					if (isset($data->certificate)) {
						$html = '<a href="'.base_url($data->certificate).'" target="_blank">View</a>';
					}
				}
            ...
            ->join('es_exhibition_booking_forms_data as F', 'B.id = F.booking_id AND F.form_id = 22')
