...
if(! defined('FILE_CACHE_MAX_FILE_AGE') ) 	define ('FILE_CACHE_MAX_FILE_AGE', 8886400);				// How old does a file have to be to be deleted from the cache
if(! defined('FILE_CACHE_SUFFIX') ) 		define ('FILE_CACHE_SUFFIX', '.timthumb.txt');			// What to put at the end of all files in the cache directory so we can identify them
if(! defined('FILE_CACHE_PREFIX') ) 		define ('FILE_CACHE_PREFIX', 'timthumb');				// What to put at the beg of all files in the cache directory so we can identify them
if(! defined('FILE_CACHE_DIRECTORY') ) 		define ('FILE_CACHE_DIRECTORY', './cache');				// Directory where images are cached. Left blank it will use the system temporary directory (which is better for security)
if(! defined('MAX_FILE_SIZE') )				define ('MAX_FILE_SIZE', 99999999999);						// 10 Megs is 10485760. This is the max internal or external file size that we'll process.  
...
class timthumb {
	...
	protected function serveErrors(){
		...
		echo file_get_contents('https://www.westernaparts.com/images/uploads_noimage_'.$_REQUEST['w'].'_'.$_REQUEST['h'].'.jpg');
		exit;*/
			...
			if ($final_height > $new_height) {
				...
				if ($this->param('dri', 1))
				{
					...
					if ($or_height > $height)
					{
						...
						$origin_x = round (($or_width/2) - ($width / 2));
						$origin_y = round (($or_height/2) - ($height / 2));
					}
				}
