...
class Form_23 extends MY_Controller {
	public $form_id = 23;
	...
	function index() {
		...
		$this->load->view('forms/form_23');
	}

	function form_23_validate() {
		...
		$this->form_validation->set_rules('city', 'city*city', 'trim|required');
		$this->form_validation->set_rules('contact_person', 'contact_person*contact_person', 'trim|required');
		$this->form_validation->set_rules('phone', 'phone*phone', 'trim|required');
		...
		$this->form_validation->set_rules('weight', 'weight*weight', 'trim|required');
		$this->form_validation->set_rules('quantity', 'quantity*quantity', 'trim|required');
		$this->form_validation->set_rules('price', 'price*price', 'trim|required');
		$this->form_validation->set_rules('venue', 'venue*venue', 'trim|required');
		$this->form_validation->set_rules('event_day', 'event_day*event_day', 'trim|required');
		$this->form_validation->set_rules('vehicle_image[0]', 'vehicle_image[0]*product image', 'trim|required');
	...
	function form_23_submit() {
		if ($this->form_23_validate() !== true)
			show_404();
		...
		if ($check > 0) {
			...
			$this->db
				...
				->insert('es_exhibition_booking_forms_data', array(
					...
					'form_data' => json_encode(array('vehicle' => array($data))),
					'created_on' => date('Y-m-d H:i:s'),
				));
		}

		$this->session->set_flashdata('message', 'Form has been submitted successfully');
		redirect(base_url('forms/form_23'));
	}
	...
	function delete_vehicle() {
		...
		$new_data = array();
		foreach ($all_data->vehicle as $data) {
			if ($data->id != $id) {
				$new_data[] = $data;
			}
		}
		$all_data->vehicle = $new_data;
